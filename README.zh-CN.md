...
2.6.0 适配 DSH 0.1.2-rc.1，保留伙伴名字、外观、位置、成长记录、纪念物、小屋方案和已有图鉴。后台成长记录写入失败时，会话仍可继续，后续事件仍可保存；失败的那次成长记录可能遗漏。手动保存失败仍会反馈给用户。会话要求保存和插件关闭时，会等待此前已接受的写入结束；日志只记录固定提示，不记录会话标识、内容或底层错误。

小须鲸重新设计为“白腹墨鲸跃水”：跃出固定水面、弯身、入水，躯干、胸鳍与尾部共同变化。24 个选定的 ImageGen 实绘姿态，加 72 张光栅补帧，以 96 帧、2.4 秒、40 fps 循环播放；系统或插件要求减少动态效果时改用同源 PNG。没有 SVG 鲸鱼，也没有用程序手绘轮廓代替生图。原始 Dive/Classic 状态动画不替换。完整提示词、来源和设计说明见 [新美术记录](docs/ink-breach-design.md)。

![白腹墨鲸跃水预览](docs/ink-breach-preview.gif)

GIF用于兼容预览；其时间精度受格式和浏览器限制。插件与[原生 WebP 动画](packages/dsh-whale-companion/assets/ink-breach-motion.webp)使用每帧25毫秒的均匀时序。
...
- **2.4.0 完整交付**：可命名伙伴、四类动态航行任务、ImageGen 栅格鲸鱼肖像、栅格精灵小须鲸动画、快速航行卡、XP 即时反馈、六套海湾主题、本地 PNG 航行名片和文本战报；运行时不再绘制矢量鲸鱼。
...
- 6 套主题调整海湾背景、界面强调与环境光，鲸鱼原画保留自身颜色；另提供 12 个成就、响应式布局、键盘操作、焦点管理和减少动画呈现。
