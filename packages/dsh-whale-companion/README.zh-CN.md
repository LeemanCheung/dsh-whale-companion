...
2.5.0 已适配 DSH 0.1.2-rc.1，保留个性化数据和已有生图图鉴。小须鲸采用新的24个墨色生图姿态与72个光栅补帧，以96帧、60fps播放；减少动态效果使用同源PNG。后台存储异常不会中断会话，后续事件仍可保存；手动保存失败仍会显示。会话要求保存与插件关闭都会等待已接受的写入结束，错误提示不包含会话内容或文件路径。
